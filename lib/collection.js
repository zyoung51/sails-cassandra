...
Collection.prototype.insert = function(items, cb) {
  ...
  items.forEach(function(item) {
    ...
    Object.keys(item).forEach(function(attrName) {
      ...
      cqlValues += "?,";
      cqlParams.push(
        myUtil.mapWaterlineTypeValueToCassandra(
          item[attrName],
          this.ddlHelper.attrTypes[attrName] )
      );
    }, this);
  ...
  this.client.eachRow(arr[0], arr[1], { prepare: true },
    function(n, row) {
      ...
      Object.keys(self.ddlHelper.colNamesToAttrNames).forEach(function(colName) {
        attrName = self.ddlHelper.colNamesToAttrNames[colName];
        attrType = self.ddlHelper.attrTypes[attrName];
        rs[attrName] = myUtil.castFromCassandraToWaterline(row[colName], attrType);
      });
      results.push(rs);
    },
...
Collection.prototype.update = function(criteria, values, cb) {
  ...
  Object.keys(values).forEach(function(attrName) {
    ...
    cqlUpdateParams.push(
      myUtil.mapWaterlineTypeValueToCassandra(
        values[attrName],
        self.ddlHelper.attrTypes[attrName] )
    );
  });
  ...
  if (idCol) {
    ...
    self.client.eachRow(cqlSelect, cqlSelectParams, { prepare: true },
      ...
      function(err, totalCount) {
        if (err) return cb(err);
        if (!queries.length) {
            console.log('update', cqlSelect, cqlSelectParams);
            return cb(new Error("Query operation is empty"));
        }
...
Collection.prototype.delete = function(criteria, cb) {
  console.log('Cass delete', criteria);
  var self = this;
  var pkColName = self.ddlHelper.attrNamesToColNames[self.ddlHelper.pkAttrName];
  var cqlSelect = 'SELECT ' + pkColName + ' FROM ' + self.ddlHelper.tableName + ' WHERE';
  ...
  if (idCol) {
    ...
    self.client.eachRow(cqlSelect, cqlSelectParams, { prepare: true },
      ...
      function(err, totalCount) {
        if (err) return cb(err);
        if (!arr.length) {
            console.log('delete', cqlSelect, cqlSelectParams);
            return cb(new Error("Query operation is empty"));
        }
        // execute batch update
    ...
    .pipe(through2({ objectMode: true },
      function(chunk, encoding, cb) {
        ...
        Object.keys(self.ddlHelper.colNamesToAttrNames).forEach(function(colName) {
          attrName = self.ddlHelper.colNamesToAttrNames[colName];
          attrType = self.ddlHelper.attrTypes[attrName];
          rs[attrName] = myUtil.castFromCassandraToWaterline(chunk[colName], attrType);
        });
        // push the result
        cb (null, rs);
      }
...
Collection.prototype.query = function(queries, params, cb) {
  ...
  if (_.isString(queries)) {
    ...
    // execute batch
    if(!batch_queries.length) {
        return cb(new Error('batch queries is empty'));
    }
...
Collection.prototype._parseDefinition = function(definition) {
  ...
  Object.keys(this.schema).forEach(function(attrName) {
    ...
    var colType = myUtil.mapWaterlineTypeToCassandra(attribute.type, attribute);
...
Collection.prototype._parseCriteria = function(value, key, parentKey) {
  ...
  if (!key) {
    ...
    // in pair
      ...
      cql = '"' + key + '" IN (?)';
      params = params.concat(value.join(','));
    }
    // modified pair
...
Collection.prototype._constructSelectQuery = function(criteria) {
  ...
  if (where) {
    ...
    if (idCol) {
      // if PK is specified then ignore all other criteria
      if (_.isString(idVal) || _.isNumber(idVal) || _.isDate(idVal) || _.isBoolean(idVal)) {
        cql += ' WHERE "' +  idCol + '" = ?';
        cqlParams = [ idVal ];
      }
      // in pair
      else if (_.isArray(idVal)) {
        cql += ' WHERE "' +  idCol + '" IN (?)';
        cqlParams = [idVal.join(',')];
      }
    }
    else {
      // parse criteria
